...
const pool = require('../../db'); // Importar o pool de conexões
...
// Endpoint adicional para buscar favoritos de um usuário
router.get('/:userId/favorites', authMiddleware.verifyToken, async (req, res) => {
  const { userId } = req.params;
  // Apenas para garantir que o usuário logado está buscando seus próprios favoritos
  if (parseInt(req.user.id) !== parseInt(userId)) {
      return res.status(403).json({ message: 'Acesso negado. Você só pode ver seus próprios favoritos.' });
  }
  try {
      const [rows] = await pool.query('SELECT post_id FROM favorites WHERE user_id = ?', [userId]);
      res.status(200).json(rows);
  } catch (error) {
      console.error('Erro ao buscar favoritos do usuário:', error);
      res.status(500).json({ message: 'Erro interno do servidor ao buscar favoritos do usuário.' });
  }
});
